...
<%@ page language="java" import="org.springframework.web.servlet.i18n.SessionLocaleResolver"%>
 ...
 <%
  ...
  if(request.getAttribute("defQCNUM") != null){
	  defQCNUM = (String) request.getAttribute("defQCNUM");
  }
  
  if(request.getAttribute("defHCNUM") != null){
	  defHCNUM = (String) request.getAttribute("defHCNUM");
  }
  
  if(request.getAttribute("defCNUM") != null){
	  defCNUM = (String) request.getAttribute("defCNUM");
  }
%>
...
<head>
	...
	function change(num) {
		if(num=="2") {
			document.getElementById("qcnumber").style.display="none";
		} else {
			document.getElementById("qcnumber").style.display='';
		}
	}
	function show() {
	...
	function check() {
		...
		if (qc.value == "" && hc.value == "" && c.value == "") {
			document.getElementById("show").innerHTML = "<spring:message code="error_qc_number_empty" />";
			document.getElementById("message").style.display='';
			return false;
		}
		if ((qc.value != "" && hc.value != "") || (qc.value != "" && c.value != "") || (c.value != "" && hc.value != "")) {
			document.getElementById("show").innerHTML = "<spring:message code="error_qc_hc_inputed" />";
			document.getElementById("message").style.display='';
			return false;
		}
	...
	function changeLan() {
		...
		form.action = "${pageContext.request.contextPath}/user/changeLan.html?local="+value;
		form.method = "post";
		form.submit();
	}
...
<body onload="init();" onkeydown="mykeydown(event)">
	<c:url var="saveUser" value="/user/login.html" />
	<div id="d1">
		<div id="d1_head">MODERN TERMINALS</div>
		<div id="d1_body">
			<form:form id="loginForm" method="post" action="${saveUser}" onsubmit="return check();">
				<input type="hidden" value="USER" name="label" id="label"/>
				<table style="margin:1% auto;" width="80%">
				<tr>
					<td align="right"><spring:message code="username" /> :</td>
					<td align="left">
						<input type="text" id="username" name="username" title="USERNAME"   style="width:85%;" />
					</td>
				</tr>
				<tr id="qcnumber">
					<td align="right"><spring:message code="qc_number" /> :</td>
					<td align="left">
						QC<input type="text" id="qc" name="qc" title="QC NUMBER" value="<%=defQCNUM %>" style="width:24%;" />
						&nbsp;&nbsp;
						HC<input type="text" id="hc" name="hc" title="HC NUMBER" value="<%=defHCNUM %>" style="width:24%;" />
						&nbsp;&nbsp;
						C<input type="text" id="c" name="c" title="C NUMBER" value="<%=defCNUM %>" style="width:24%;" />
					</td>
				</tr>
				<tr>
					<td align="right"><spring:message code="password" /> :</td>
					<td align="left">
						<input type="password" id="password" name="password" title="PASSWORD"  style="width:85%;" />
					</td>
				</tr>
				<tr>
